...
| `writer.py` | Writes the 13 output files (matches sampled to 10, mismatches, keymismatch_A/B, dups_A/B, dups_A/B_count_report.csv, report.csv, summary.json, compare_reports.csv, compare_reports.html, keys_mismatch_matrix.csv). Each run lands in its own `report-…` subdir (ADR-037). The HTML report includes a "Sample records" section — capped examples of matches / mismatches (A↔B) / dups (with links to the per-key dup-count CSVs) / orphans (ADR-040). Segment-alias rules are NOT shown in the report (backend concern). |
